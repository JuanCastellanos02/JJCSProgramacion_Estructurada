...
int main(){
    ...
    FILE *FP;
	FP=fopen("Programa14.txt", "w");
	fprintf(FP, "\n\t\t\t REGISTRO COMPLETO DE LOS ALUMNOS");
	for(i=0; i<alumno; i++){
		fprintf(FP, "\n\t Id: %s", alumnos[i].id);
		fprintf(FP, "\n\t Nombre: %s", alumnos[i].id);
		fprintf(FP, "\n\t Apellido1: %s", alumnos[i].id);
		fprintf(FP, "\n\t Apellido2: %s", alumnos[i].id);
		fprintf(FP, "\n\t Nacimiento: %s", alumnos[i].id);
		fprintf(FP, "\n\t Escuela: %s", alumnos[i].id);
		fprintf(FP, "\n\t Grado: %s", alumnos[i].id);
		fprintf(FP, "\n\t Carrera: %s", alumnos[i].id);
		fprintf(FP, "\n\t Sexo: %s", alumnos[i].id);
	}
	fclose(FP);
